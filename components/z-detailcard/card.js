...
export default class ZDetailCard extends React.PureComponent {
  ...
  render () {
    ...
    return (
      <div className={cx('detail-card-wrapper')}>
          ...
          return (
            <Row gutter={16} className={cx('card-row')} key={rowIndex}>
              {detailCardRow.map((item, colIndex) => {
                let { label, content, colSpan, type = 'string', required = false, labelPadding = true, isShow = true, colon = true } = item
                if (!isShow) return
                return (
                  <Col span={colSpan} className={cx('card-col')} key={colIndex}>
                    <span className={cx('card-label', { required, 'label-padding': labelPadding })}>{label}{colon ? '：' : ''}</span>
                    {type == 'phone' &&
                      <ZPhoneWrapper value={content} />
                    }
                    {type == 'string' &&
                      ...
                      <div className={cx('card-image-content')}>
                        {content && content.length && content.length != 0
                          ...
                          : <div className={cx('image-empty')}>
                            <img src='https://oss-static.eaiot.cloud/aiot-web/2021-04-13-none／photo@2x-1618291984280.png' />
                            <span>暂无图片</span>
                          </div>
                        }
                      </div>
